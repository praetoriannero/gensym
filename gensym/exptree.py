...
class BinaryOperator(Operator):
    ...
    def to_string(self, graph: nx.DiGraph) -> str:
        ...
        return str(child_a.to_string(graph)) + f" {self._op} " + str(child_b.to_string(graph))
...
class Variable(Leaf):
    ...
    def __init__(self, data: np.ndarray, column: int | None=None, **kwargs):
        self.data = data
        self.col = column if column is not None else np.random.randint(0, data.shape[-1])
        super().__init__(**kwargs)
...
class ExpressionTree:
    ...
    def _grow(self, node: Node, depth: int) -> None:
        ...
        depth += 1
        child_nodes = [random.choice(node_types)(data=self.data) for _ in range(node.child_count)]
